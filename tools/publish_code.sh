...
if [ -z $APP_VERSION ]; then
    ...
    APP_DIR=$INSTALL_DIR_APP'/php-release'/$APP_VERSION
    debug 'clone from '$GIT_CODE' to '$APP_DIR
    git clone $GIT_CODE $APP_DIR
    chown -R $ENV_USER:$ENV_GROUP $APP_DIR
    chmod -R 0775 $APP_DIR
    info 'code for '$APP_NAME' have been cloned to '$APP_DIR
else
    # switch version
    APP_DIR=$INSTALL_DIR_APP'/php-release/'$APP_VERSION
    if [ -d $APP_DIR ]; then
        VERSION_FILE=$INSTALL_DIR_APP'/version/code_'$APP_NAME'.ver'
        debug 'write '$APP_VERSION' to '$VERSION_FILE
        echo $APP_VERSION > $VERSION_FILE
        info 'code version of '$APP_NAME' switched to '$APP_VERSION
    else
